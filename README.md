...
> **断线免疫（v3 新增）**：平时**完全按原设计前台执行**（实时输出、私钥默认保留在服务器、"输入 yes 关密码登录"交互全部照旧）。只有 SSH 连接**断开的一瞬间**，脚本自动转入后台续跑（幂等，从断点继续），日志在 `/root/install_reality.log`（`INSTALL_LOG` 可覆盖），重连后 `tail -f /root/install_reality.log` 查看进度。后台续跑时：**不自动关闭密码登录**（防锁死），私钥保留在 `/root/.ssh/id_ed25519`；重连后执行 **`sshkey`** 一键完成"查看私钥（确认保存后才删除）+ 检查/关闭密码登录（先确认后关闭）"。
...
| **sshkey 快捷指令** | 重连后执行 `sshkey`：查看 SSH 私钥（**确认已保存后才删除**，或 `sshkey --delete` 直接删）+ **检查密码登录**（未关时会先确认、输入 `yes` 才自动关闭并校验，已关则提示）；`sshkey --check` 只检查不修改（`showsshkey` 是它的别名）| 通用 |
...
| **关闭密码登录** | 前台交互式：现场验证 `sshd -T` 为 `no` 后输入 `yes` 才生效；`sshkey` 同样先确认、输入 `yes` 才关闭；**断连后台续跑时不自动关闭**（保持密码登录防锁死）。**安全闸门**：服务器上私钥文件不存在时绝不关闭密码登录（防止本地没存私钥就关→永久锁死）| 通用 |
...
> 注意：**交互需要真实终端**。前台正常执行时照常交互（`/dev/tty` 读取，管道方式也能用）。**断连转后台续跑时**没有终端：脚本**不自动关闭密码登录**（保持现状，防锁死）；重连后执行 `sshkey`，它会显示私钥（确认保存后才删除）并检查密码登录——未关时会先确认、输入 `yes` 才关闭（带私钥、`authorized_keys` 校验），已关就提示一下。
...
3. **私钥（默认保留，不自动删除）**：脚本生成 ed25519 私钥后**默认保留在服务器** `/root/.ssh/id_ed25519`（权限600），前台和断连续跑都不再自动删除——这样即使你"没有输出/中途断线/没看清屏幕"，私钥也不会永久丢失。这是**防锁死**的关键设计。重连后执行 `sshkey` 查看私钥，**确认已在 Termius 保存后**才删除（会问你输入 `yes` 才删；也可用 `sshkey --delete` 直接删），并请清理安装日志里的私钥段。`sshkey` 同时会检查/关闭密码登录（未关时先确认、输入 `yes` 才自动关闭并校验，已关则提示）；`sshkey --check` 可只查不改。
   > **注意（统一确认规则）**：脚本里**所有**关闭密码登录的入口（主脚本末尾那步、以及 `sshkey`）都必须**输入 `yes` 才会真正关闭**，回车或任意键都只是跳过、保持不变——不会被悄悄关掉。并且**未确认时连关密码的配置文件都不会写入**（先确认、后才写配置、才重启 sshd），所以不会出现"回车跳过、重进却已关闭"的情况。`sshkey` 的密码检查段同样遵循此规则：发现密码未关时会先问你确认，`yes` 才关，其余键保留。这些入口在关闭前还会检查私钥文件是否还在服务器上，不在就绝不关（防止本地没存私钥就关→永久锁死）。
...
8. **遇到"没有输出 / 屏幕空白 / 连接失败 / 验证失败"先排查，别急着重跑**：重连后按下面顺序逐项检查（这也是本脚本断线免疫设计的一部分）：
   ...
   4. `exec sshkey --check` —— 只检查密码登录状态、不改动；确认私钥文件 `/root/.ssh/id_ed25519` 还在不在（默认保留，但若曾被手动删过则需注意）。
...
| `/usr/local/bin/sshkey` | `sshkey` 命令本体（查看私钥 + 检查/关闭密码登录，关闭前需确认）|
