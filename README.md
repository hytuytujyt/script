...
> **断线免疫（v3 新增）**：平时**完全按原设计前台执行**（实时输出、私钥默认保留在服务器、"输入 yes 关密码登录"交互全部照旧）。只有 SSH 连接**断开的一瞬间**，脚本自动转入后台续跑（幂等，从断点继续），日志在 `/root/install_reality.log`（`INSTALL_LOG` 可覆盖），重连后 `tail -f /root/install_reality.log` 查看进度。后台续跑时：**不自动关闭密码登录**（防锁死），私钥保留在 `/root/.ssh/id_ed25519`；重连后执行 **`sshkey`** 一键完成"查看私钥（确认保存后才删除）+ 自动检查/关闭密码登录"。
...
| **sshkey 快捷指令** | 重连后执行 `sshkey`：查看 SSH 私钥（**确认已保存后才删除**，或 `sshkey --delete` 直接删）+ **自动检查密码登录**（没关就自动关闭并校验，已关则提示）；`sshkey --check` 只检查不修改（`showsshkey` 是它的别名）| 通用 |
...
| **关闭密码登录** | 前台交互式：现场验证 `sshd -T` 为 `no` 后输入 `yes` 才生效；**断连后台续跑时不自动关闭**（保持密码登录防锁死），重连后执行 `sshkey` 自动检查并关闭。**新增安全闸门**：服务器上私钥文件不存在时绝不关闭密码登录（防止本地没存私钥就关→永久锁死）| 通用 |
...
3. **私钥（默认保留，不自动删除）**：脚本生成 ed25519 私钥后**默认保留在服务器** `/root/.ssh/id_ed25519`（权限600），前台和断连续跑都不再自动删除——这样即使你"没有输出/中途断线/没看清屏幕"，私钥也不会永久丢失。这是**防锁死**的关键设计。重连后执行 `sshkey` 查看私钥，**确认已在 Termius 保存后**才删除（会问你输入 `yes` 才删；也可用 `sshkey --delete` 直接删），并请清理安装日志里的私钥段。`sshkey` 同时会自动检查/关闭密码登录（未关则自动关闭并校验，已关则提示）；`sshkey --check` 可只查不改。
   > **为什么默认保留而不是打印一次即删？** 因为脚本运行到末尾会提示你输入 `yes` 关闭密码登录。一旦关闭密码，唯一登录途径就是私钥验证。如果私钥已经被删除/你没保存，一关就永久锁死（SSH 报"验证失败"）。所以私钥默认保留，且关闭密码登录前脚本会检查私钥文件是否还在——私钥不在就绝不允许关密码。
