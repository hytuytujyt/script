...
> **断线免疫（v3 新增）**：管道方式执行时，脚本会自动转入**后台安装**并立即返回——SSH 中途断开不影响安装。安装日志默认在 `/root/install_reality.log`（可用 `INSTALL_LOG` 覆盖），随时重连执行 `tail -f /root/install_reality.log` 查看进度。想前台实时看输出、或保留"关闭密码登录"的交互确认，加 `INSTALL_FOREGROUND=1` 重新执行。
...
| `INSTALL_LOG` | 断线免疫后台模式的安装日志路径（默认 `/root/install_reality.log`）| `INSTALL_LOG=/root/my.log` |
| `INSTALL_FOREGROUND` | 设为 `1` 强制前台执行（实时输出 + 保留交互确认，关闭断线免疫）| `INSTALL_FOREGROUND=1` |
...
| **断线免疫** | 管道/无终端模式自动转入后台安装（nohup），SSH 断开不影响；日志 `/root/install_reality.log`，`INSTALL_FOREGROUND=1` 强制前台 | 通用 |
...
| **关闭密码登录** | 交互式：现场验证 `sshd -T` 为 `no` 后输入 `yes` 才生效；**无终端（后台模式）时自动跳过**，保持密码登录 | 通用 |
...
> 注意：**交互需要真实终端**。v3 起管道模式（`curl ... | sh`）会自动转入后台安装（断线免疫），没有终端 → 此交互步骤自动跳过、保持密码登录；想走完整交互流程，请下载到本地前台执行 `sh install_reality.sh`（或加 `INSTALL_FOREGROUND=1`）。
...
4. **小内存机器**：sing-box 仅占几十 MB，200MB 配额足够；面板显示的高占用多为文件缓存，无需处理。安装过程（下载解压二进制）内存峰值较高、耗时较长——断线免疫已自动后台执行，不怕 SSH 中途断开。
