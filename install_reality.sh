...
disable_password_login() {
  ...
  echo "  2) 执行:  sshd -T | grep -i passwordauthentication"
  echo "     应看到:  PasswordAuthentication no"
  echo "     还要看到: KbdInteractiveAuthentication no"
  echo "               ChallengeResponseAuthentication no"
  echo "确认以上都是 no 后,输入 yes 才会真正关闭并重启 sshd;"
  ...
  echo "  PasswordAuthentication=$pa  KbdInteractiveAuthentication=$kbd  ChallengeResponseAuthentication=$cr"
  if [ "$pa" = "no" ] && [ "$kbd" = "no" ] && [ "$cr" = "no" ]; then
    _ok2 "校验通过:密码登录已确认为关闭。"
  else
    _err2 "校验失败:密码登录可能仍为开启(pa=$pa kbd=$kbd cr=$cr)。"
    _err2 "已保留配置,请手动复核: sshd -T | grep -i authentication"
  fi

  # --- ⑦ 最终你肉眼复核 ---
  echo ""
  _info2 "请再肉眼确认一次以下输出(应都是 no):"
