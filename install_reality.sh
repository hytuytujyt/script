...
#
#  v3 新增【断线免疫】:管道/无终端模式(如 curl ... | sh)自动转入后台安装,
#  安装进程与 SSH 会话解耦,SSH 断开不影响安装;日志默认 /root/install_reality.log
#  (可用环境变量 INSTALL_LOG 覆盖),随时 tail -f 查看进度;
#  想前台跑看实时输出/保留交互确认,加 INSTALL_FOREGROUND=1。
# =====================================================================
...
# ---------- 断线免疫:管道/无终端模式自动转入后台执行 ----------
# 场景:  curl ... | sh 、sh install_reality.sh < /dev/null 等非交互方式。
# 效果:  安装进程与 SSH 会话解耦,SSH 断开不影响安装;日志见 INSTALL_LOG。
# 关闭:  设 INSTALL_FOREGROUND=1 强制前台执行(实时输出/保留交互确认)。
if [ -z "${INSTALL_DAEMONIZED:-}" ] && [ -z "${INSTALL_FOREGROUND:-}" ] && [ ! -t 0 ]; then
  _self=""
  if [ -n "$0" ] && [ -f "$0" ]; then
    _self="$0"
  else
  ...
  if [ -n "$_self" ]; then
    ...
    printf '%s\n' "[*] 检测到管道/非交互模式,安装已转入后台执行(SSH 断开不影响安装)。"
    printf '%s\n' "[*] 安装日志: $_log"
    printf '%s\n' "[*] 查看进度: tail -f $_log"
    printf '%s\n' "[*] 需要前台实时输出时: INSTALL_FOREGROUND=1 重新执行"
    exit 0
  fi
  # 后台化兜底失败(如无下载工具),继续走原有前台引导流程
fi
...
EOF
      ...
      _err2 "openrc 安装后仍不可用,请手动启动 sing-box: $bin_path run -c $CONFIG_FILE"
    fi
  fi
}
