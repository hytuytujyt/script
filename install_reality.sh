...
### 2.2 安装 jq(仅用于合并配置,保留已有的其它节点如 argo,不影响其它功能) ###
install_jq() {
  if command -v jq >/dev/null 2>&1; then
    _ok2 "jq 已安装,跳过"
    return
  fi
  _info2 "安装 jq(用于合并配置,保留已有的其它节点如 argo) ..."
...
# 逻辑: 若 $CONFIG_FILE 已存在且是合法 JSON -> 先备份 -> 只删除 tag 为
#       reality-in / ss-in 的旧 inbound(两者互斥,不管这次跑哪个模式都先清掉,
#       避免残留一个不再使用的旧节点) -> 其余 inbound(包括手写的 argo vless-ws,
#       不管它 tag 是什么)原样保留 -> 插入本次新生成的 inbound -> 用 sing-box
#       check 校验通过才落盘,校验失败则放弃本次写入、报错退出,原文件不受影响。
...
merge_write_config() {
  ...
  if [ -s "$CONFIG_FILE" ] && jq empty "$CONFIG_FILE" >/dev/null 2>&1; then
    ...
    _info2 "检测到已有配置,已备份到 $backup(仅替换 reality/ss 节点,其余节点如 argo 保持不动)"

    local tmpfile="${CONFIG_FILE}.tmp.$$"
    if ! jq --argjson newin "$new_inbound_json" '
        .inbounds = ((.inbounds // []) | map(select(.tag != "reality-in" and .tag != "ss-in"))) + [$newin]
