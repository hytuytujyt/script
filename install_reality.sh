...
# 关闭前验证①: 服务器上私钥文件必须还在(关了密码后唯一登录途径就是私钥)
if [ ! -f "$KEY" ]; then
  echo "[!] 服务器上私钥文件($KEY)不存在,关闭密码登录会锁死自己,不自动关闭"
  exit 0
fi
...
# 关闭前确认: 与主脚本末尾那步保持一致——必须输入 yes 才关,回车/任意键都不关(防误关锁死)
echo ""
echo "===== 确认关闭密码登录 ====="
echo "密码登录当前为开启状态 (PasswordAuthentication=$PA)。"
echo "关闭后将无法通过密码登录,只能靠密钥。"
REPLY=""
printf '%s' "确认要关闭? 输入 yes 回车执行;输入其他任意键则保留密码登录 > "
...
if [ "$REPLY" != "yes" ]; then
  echo ""
  echo "[*] 未确认,保留密码登录 (PasswordAuthentication=$PA)"
  exit 0
fi
...
disable_password_login() {
  ...
  # --- ① 交互确认(放在最前):未确认绝不写入任何配置、绝不触碰 sshd ---
  # 原实现把"写入 drop-in + include"放在确认之前,导致即使回车跳过确认,
  # 配置文件也已落盘,之后 sshd 一重载(系统/面板重启)就悄悄关闭密码登录。
  # 现改为:只有输入 yes 确认后,才真正写配置、才重启 sshd。
  echo ""
  echo "================ 关闭密码登录前请现场确认 ================"
  ...
  echo "==========================================================="
  local confirm=""
  # 从真实终端读取交互确认。优先用 /dev/tty(直接指向 SSH 终端,管道模式也适用);
  # 若当前会话没有控制终端(/dev/tty 打不开,如某些面板/非 tty 连接),
  # 只要 stdin 本身是终端就退一步从 stdin 读取,仍可能正常交互;
  # 两者都不可用才跳过(non-interactive,保持现状不关)。
  if { exec 3<>/dev/tty; } 2>/dev/null; then
    printf '%s' "请输入 yes 关闭密码登录 > "
    read -r confirm <&3 || true
    exec 3<&-
  elif [ -t 0 ]; then
    printf '%s' "请输入 yes 关闭密码登录 (stdin 交互) > "
    read -r confirm || true
  else
    _info2 "未检测到可交互终端(/dev/tty)且 stdin 不是终端,跳过关闭密码登录(保持现状)。"
  fi
  if [ "$confirm" != "yes" ]; then
    _warn2 "未确认,跳过关闭密码登录(保持现状,仍可用密码登录)。配置未改动。"
    return
  fi

  # --- ② 确认后才写入最优先的 drop-in(00- 确保 first-match-wins 生效) ---
  mkdir -p "$drop_dir"
  printf '%s\n' \
