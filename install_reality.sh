...
setup_alias() {
  ...
  # 写入 sshkey 独立脚本: 仅查看/确认删除 SSH 私钥(关闭密码登录已移出,交由独立脚本处理)
  cat > /usr/local/bin/sshkey <<'SSHKEYEOF'
#!/bin/sh
# sshkey: 查看 SSH 私钥(确认保存后才删除)。仅保留私钥逻辑。
# 用法:
#   sshkey              显示私钥;输入 yes 确认已保存后删除服务器上的私钥
#   sshkey --delete     显示并直接删除私钥(不询问)
