  ...
  _restart_daemon() {
    ...
    if [ -n "$0" ] && [ -f "$0" ]; then
      ...
      if command -v curl >/dev/null 2>&1; then
        curl -fsSL "$SCRIPT_URL" -o "$_tmp_self" 2>/dev/null || rm -f "$_tmp_self"
      elif command -v wget >/dev/null 2>&1; then
        wget -qO "$_tmp_self" "$SCRIPT_URL" 2>/dev/null || rm -f "$_tmp_self"
      elif command -v busybox >/dev/null 2>&1; then
        busybox wget -qO "$_tmp_self" "$SCRIPT_URL" 2>/dev/null || rm -f "$_tmp_self"
      fi
...
if [ -z "${BASH_VERSION:-}" ]; then
  ...
  # --- 内核选择:由主正文段 decide_kernel 检测 /tmp 是否内存盘 + 内存大小,并让你手动选择 xray / sing-box ---
  # 这里不再预设 KERNEL(否则主正文段会因 KERNEL 已设置而跳过手动选择)。
...
# --- 内核选择:检测 /tmp 是否内存盘 + 内存大小,并让你手动选择 xray / sing-box ---
# 显式设置 KERNEL(sing-box / xray)则直接沿用,跳过提问(重跑/脚本化友好)。
# 有交互终端 → 展示检测结果,输 1/2;回车 / 非法输入 → 按内存给的安全默认。
# 无交互终端(管道/cron)→ 直接按内存默认,不阻塞。
decide_kernel() {
  if [ -n "${KERNEL:-}" ]; then
    case "$KERNEL" in
      sing-box|xray) return 0 ;;
      *) _warn2 "KERNEL=$KERNEL 非法,改由检测/选择决定"; unset KERNEL ;;
    esac
  fi

  local tmpfs="no" mem_mb=""
  grep -qE " /tmp tmpfs " /proc/mounts 2>/dev/null && tmpfs="yes"
  mem_mb=$(free -m 2>/dev/null | awk '/^Mem:/{print $2}') || true
  [ -z "$mem_mb" ] && mem_mb="未知"

  # 安全默认: 内存 >=1GB → sing-box,否则 → xray
  local def="sing-box"
  if [ -n "$mem_mb" ] && [ "$mem_mb" -eq "$mem_mb" ] 2>/dev/null && [ "$mem_mb" -lt 1024 ]; then
    def="xray"
  fi

  printf '\n%s\n' "[检测] /tmp 是否为内存盘(tmpfs): $tmpfs"
  printf '%s\n' "[检测] 总内存:                     ${mem_mb} MB"
  echo "请选择要安装的内核:"
  echo "   [1] sing-box   (功能更全,含 prefer_ipv6)"
  echo "   [2] xray       (体积更小,省内存)"

  local pick=""
  if { exec 3<>/dev/tty; } 2>/dev/null; then
    printf '输入 1 或 2 (回车=自动建议[%s]) > ' "$def" >&3
    read -r pick <&3 || true
    exec 3<&- 3>&-
    pick=$(printf '%s' "${pick:-}" | tr -d '[:space:]' | tr '[:upper:]' '[:lower:]')
    case "$pick" in
      1|singbox|sing-box) KERNEL="sing-box" ;;
      2|xray)             KERNEL="xray" ;;
      *)                  KERNEL="$def" ;;   # 回车 / 非法 → 安全默认
    esac
  else
    _info2 "未检测到可交互终端,按内存自动选择: $def"
    KERNEL="$def"
  fi
  _info2 "已选择内核: $KERNEL"
}
...
# 内核选择:检测 /tmp 是否内存盘 + 内存大小,手动选 xray / sing-box(显式 KERNEL 则直接沿用,不提问)
decide_kernel
...
install_singbox() {
  ...
  tmpdir="/var/tmp/singbox-install.$$"
  mkdir -p "$tmpdir"
...
setup_ssh_key() {
  ...
  if [ -n "${SSH_KEY_DELETE:-}" ] || [ -z "${INSTALL_DAEMONIZED:-}" ]; then
    # 原设计(前台正常执行) 或 强制删除: 私钥打印一次即删
    echo ""
    ...
    # 断连转后台续跑: 用户看不到屏幕,私钥保留在服务器防止丢失(同时已打印进日志)
    echo ""
    echo "================ SSH 私钥(断连续跑,已保留在服务器) ================"
    cat "$key_file"
    echo "========================================================================="
    echo ""
    _ok2 "SSH 公钥已写入 $auth_file;私钥已保留在 $key_file (权限600)"
    _info2 "重连后执行 sshkey: 查看私钥(查看后自动删除)并自动检查/关闭密码登录"
    _info2 "后台续跑不自动关闭密码登录(防锁死),由你重连后执行 sshkey 确认后关闭"
  fi
