...
SERVER_LISTEN_PORT=443                                        # relay 模式 reality 实际监听端口,恒定443
SS_LISTEN_PORT=8388                                           # landing 模式 Shadowsocks 监听端口(默认)
...
# 按模式确定实际监听端口(reality 恒定443;ss 默认8388,均可被 LISTEN_PORT 映射覆盖)
if [ "$MODE" = "landing" ]; then
  SERVER_LISTEN_PORT=$SS_LISTEN_PORT
fi
...
# 注意: 落地模式下服务端监听端口必须 = 服务商给你的固定端口(或默认8388)。
# 因此这里用 OUTPUT_PORT(即用户指定的 LISTEN_PORT),而不是常量 443。
write_ss_config() {
  mkdir -p "$CONFIG_DIR"
  cat > "$CONFIG_FILE" <<EOF
{
...
EOF
  _ok2 "Shadowsocks 配置已写入 $CONFIG_FILE (监听端口 $OUTPUT_PORT)"
}

### 4. 检查并放行防火墙 ###
check_and_open_firewall() {
  # landing 模式放行用户指定的固定端口(OUTPUT_PORT);relay 模式放行常量 443
  local port=$OUTPUT_PORT
