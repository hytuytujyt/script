...
robust_download() {
  ...
  while [ "$attempt" -le 5 ]; do
    ...
    if curl -fL -C - --retry 3 --retry-delay 2 --connect-timeout 20 -o "$out" "$url" 2>/dev/null; then
      return 0
    fi
    attempt=$((attempt+1))
    sleep 2
...
{
  "log": { "loglevel": "warning", "output": "/var/log/xray.log" },
  "inbounds": [
    {
...
setup_logrotate() {
  local svc_name
  if [ "$KERNEL" = "xray" ]; then svc_name="xray"; else svc_name="sing-box"; fi
...
EOF
  ...
  sleep 2
  local code
  code=$(curl -sS -m 10 -x "socks5h://127.0.0.1:${port}" -o /dev/null -w "%{http_code}" https://www.google.com/generate_204 2>/dev/null || true)
  if [ "$code" = "204" ]; then
    _ok2 "服务端自检通过:Reality 握手 + 出站正常"
  else
    _warn2 "服务端自检未通过(http=$code),请检查: rc-service $svc status; tail -n 20 $logf"
  fi
...
setup_alias() {
  ...
  # log 快捷指令按所选内核指向对应日志文件(xray 分支日志在 /var/log/xray.log)
